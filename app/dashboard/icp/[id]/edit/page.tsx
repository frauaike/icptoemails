...
export default function EditICPPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchICP = async () => {
      try {
        ...
        const data: ICP = await response.json()
        setFormData({
          name: data.name,
          industry: data.industry,
          companySize: data.companySize,
          personaTitle: data.personaTitle,
          personaResponsibilities: data.personaResponsibilities,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.detail || "Failed to update ICP")
      }

      toast.success("ICP updated successfully!")
      router.push(`/dashboard/icp/${params.id}`)
    } catch (error) {
  ...
  return (
    <DashboardShell>
      <DashboardHeader heading="Edit ICP" text="Update your ideal customer profile details.">
        <Button variant="outline" onClick={() => router.push(`/dashboard/icp/${params.id}`)} className="border-gray-800">
